...
function finish() // ამოწმებს, დამთავრდა თამაში თუ არა.
{
	...
	for (let i = 0;//i  gamePiece.length;//i++) // პაზლის თითოეული ფრაგმენტისთვის
	{
		...
		// ამოწმებს, შეესაბამება ეს ფრაგმენტი თუ არა ზემო და მარცხენა პოზიციას.
		{
			flag = false;
			break;
		}
...
function left(x, y) // თვლის, რამდენამ შორს მარცნიდან უნდა იყოს განთავსებული პაზლის ფრაგმენტი.
{
	...
	if (cordX  0) // თუ X-კოორდინატა ნულის ტოლია,
	{
		for (let i = 0;//i  gamePiece.length;//i++) // ყველა ფრაგმენტებისთვის შესრულდება შემდეგი კოდი.
	{
		// თუ ფრაგმენტის კოორდინატა მარცხნიდან + 100 ტოლია cordX-სა და კოორდინატა ზემოდან ტოლია cordY-სა,
...
function up(x, y) // თვლის, რამდენამ შორს ზემოდან უნდა იყოს განთავსებული პაზლის ფრაგმენტი.
	...
	if (cordY  0) // თუ cordY-ის მნიშვნელობა ნულის ტოლია,
	{
		...
		// თუ ფრაგმენტის კოორდინატა ზემოდან + 100 ტოლია cordY-სა და კოორდინატა მარცხნიდან ტოლია cordX-სა,
		// დაგვიბრუნდება ფრაგმენტის ნომერი.
...
{
	let temp = gamePiece[position].style.top; // დროებით ცვლადს ვანიჭებთ ფრაგმენტის ზემოდან კოორდინატას.

	gamePiece[position].style.top = spaceY; // ფრაგმენტის ზემოდან კოორდინატას ვანიჭებთ spaceY-ს.
	...
	gamePiece[position].style.left = spaceX; // მარჯვენა კოორდინატას ვანიჭებთ spaceX-ის მნიშვნელობას.
